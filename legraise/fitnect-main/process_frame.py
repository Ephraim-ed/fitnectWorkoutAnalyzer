...
class ProcessFrame:
    ...
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.FEEDBACK_ID_MAP = {
                                0: ('GOOD LEG RAISE', 150, (0, 153, 255)), #2
                                1: ('RAISE YOUR THIGH', 181, (0, 153, 255)),#1
                                2: ('CORRECT', 160, 170, (255, 80, 80)),
                                3: ('OVER RAISE', 181, (255, 80, 80))#3
                               }
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            offset_angle = find_angle(left_knee_coord, right_knee_coord, nose_coord)
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                if self.state_tracker['INACTIVE_TIME_FRONT'] >= self.thresholds['INACTIVE_THRESH']:
                    self.state_tracker['LEGRAISE_COUNT'] = 0
                    self.state_tracker['IMPROPER_LEGRAISE'] = 0
                ...
                if display_inactivity:
                    cv2.putText(frame, 'Resetting LEGRAISE_COUNT due to inactivity', (10, frame_height - 90), self.font, 0.5, self.COLORS['blue'], 2, lineType=self.linetype)
                ...
                draw_text(
                    frame, 
                    "CORRECT: " + str(self.state_tracker['LEGRAISE_COUNT']), 
                ...
                draw_text(
                    frame, 
                    "INCORRECT: " + str(self.state_tracker['IMPROPER_LEGRAISE']), 
            ...
            # Camera is aligned properly.
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        self.state_tracker['LEGRAISE_COUNT']+=1
                        play_sound = str(self.state_tracker['LEGRAISE_COUNT'])
                        
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq'])==1:
                        self.state_tracker['IMPROPER_LEGRAISE']+=1
                ...
                cv2.putText(frame, str(int(hip_vertical_angle)), (hip_text_coord_x, hip_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(knee_vertical_angle)), (knee_text_coord_x, left_knee_coord[1]+10), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(knee_vertical_angle)), (knee_text_coord_x, right_knee_coord[1]+10), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(ankle_vertical_angle)), (ankle_text_coord_x, ankle_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
            ...
            if self.state_tracker['INACTIVE_TIME'] >= self.thresholds['INACTIVE_THRESH']:
                self.state_tracker['LEGRAISE_COUNT'] = 0
                self.state_tracker['IMPROPER_LEGRAISE'] = 0
                cv2.putText(frame, 'Resetting LEGRAISE_COUNT due to inactivity!!!', (10, frame_height - 25), self.font, 0.7, self.COLORS['blue'], 2)
                display_inactivity = True
