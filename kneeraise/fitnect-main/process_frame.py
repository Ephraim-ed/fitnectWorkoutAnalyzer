...
class ProcessFrame:
    ...
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.state_tracker = {
            ...
            'KNEERAISE_COUNT': 0,
            'IMPROPER_KNEERAISE':0
            
        }
        
        self.FEEDBACK_ID_MAP = {
                                0: ('GOOD KNEE RAISE', 215, (0, 153, 255)),#2
                                1: ('RAISE YOUR KNEE HIGHER', 181, (0, 153, 255)),#1
                                2: ('CORRECT', 160, 170, (255, 80, 80)),
                                3: ('OVER KNEE RAISE', 181, (255, 80, 80))#3
                               }
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, lower_knees_disp):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                if self.state_tracker['INACTIVE_TIME_FRONT'] >= self.thresholds['INACTIVE_THRESH']:
                    self.state_tracker['KNEERAISE_COUNT'] = 0
                    self.state_tracker['IMPROPER_KNEERAISE'] = 0
                ...
                if display_inactivity:
                    cv2.putText(frame, 'Resetting KNEERAISE_COUNT due to inactivity', (10, frame_height - 90), self.font, 0.5, self.COLORS['blue'], 2, lineType=self.linetype)
                ...
                draw_text(
                    frame, 
                    "CORRECT: " + str(self.state_tracker['KNEERAISE_COUNT']), 
                ...
                draw_text(
                    frame, 
                    "INCORRECT: " + str(self.state_tracker['IMPROPER_KNEERAISE']), 
            ...
            # Camera is aligned properly.
                ...
                #ankle_vertical_angle = find_angle(right_knee_coord, np.array([right_ankle_coord[0], 0]), right_ankle_coord)
                #cv2.ellipse(frame, right_ankle_coord, (30, 30),
                #            angle = 0, startAngle = -90, endAngle = -90 + multiplier*right_ankle_vertical_angle,
                #            color = self.COLORS['white'], thickness = 3,  lineType=self.linetype)
                ...
                current_state = self._get_state(int(left_knee_vertical_angle))
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        self.state_tracker['KNEERAISE_COUNT']+=1
                        play_sound = str(self.state_tracker['KNEERAISE_COUNT'])
                        
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq'])==1:
                        self.state_tracker['IMPROPER_KNEERAISE']+=1
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    if left_hip_vertical_angle > self.thresholds['LEFT_HIP_THRESH'][1]:
                        self.state_tracker['DISPLAY_TEXT'][0] = True
                        

                    elif left_hip_vertical_angle < self.thresholds['LEFT_HIP_THRESH'][0] and \
                         self.state_tracker['state_seq'].count('s2')==1:
                            self.state_tracker['DISPLAY_TEXT'][1] = True
                        
                    if right_hip_vertical_angle > self.thresholds['RIGHT_HIP_THRESH'][1]:
                        self.state_tracker['DISPLAY_TEXT'][0] = True
                        

                    elif right_hip_vertical_angle < self.thresholds['RIGHT_HIP_THRESH'][0] and \
                         self.state_tracker['state_seq'].count('s2')==1:
                            self.state_tracker['DISPLAY_TEXT'][1] = True
                        
                                        
                    
                    if self.thresholds['LEFT_KNEE_THRESH'][0] < left_knee_vertical_angle < self.thresholds['LEFT_KNEE_THRESH'][1] and \
                       self.state_tracker['state_seq'].count('s2')==1:
                        self.state_tracker['LOWER_KNEES'] = True


                    elif left_knee_vertical_angle > self.thresholds['LEFT_KNEE_THRESH'][2]:
                        self.state_tracker['DISPLAY_TEXT'][3] = True
                        self.state_tracker['INCORRECT_POSTURE'] = True

                    if self.thresholds['RIGHT_KNEE_THRESH'][0] < right_knee_vertical_angle < self.thresholds['RIGHT_KNEE_THRESH'][1] and \
                       self.state_tracker['state_seq'].count('s2')==1:
                        self.state_tracker['LOWER_KNEES'] = True


                    elif right_knee_vertical_angle > self.thresholds['RIGHT_KNEE_THRESH'][2]:
                        self.state_tracker['DISPLAY_TEXT'][3] = True
                        self.state_tracker['INCORRECT_POSTURE'] = True


                    #if (left_ankle_vertical_angle > self.thresholds['LEFT_ANKLE_THRESH']):
                    #    self.state_tracker['DISPLAY_TEXT'][2] = True
                     #   self.state_tracker['INCORRECT_POSTURE'] = True
                        
                    #elif (right_ankle_vertical_angle > self.thresholds['RIGHT_ANKLE_THRESH']):
                       # self.state_tracker['DISPLAY_TEXT'][2] = True
                      #  self.state_tracker['INCORRECT_POSTURE'] = True
                ...
                right_hip_text_coord_x = right_hip_coord[0] + 10
                left_knee_text_coord_x =left_knee_coord[0] + 15
                right_knee_text_coord_x =right_knee_coord[0] + 15
                right_ankle_text_coord_x = left_ankle_coord[0] + 10
                right_ankle_text_coord_x = right_ankle_coord[0] + 10
                ...
                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['LOWER_KNEES'] = False

                self.state_tracker['COUNT_FRAMES'][self.state_tracker['DISPLAY_TEXT']]+=1

                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['LOWER_KNEES'])



                if display_inactivity:
                    cv2.putText(frame, 'Resetting COUNTERS due to inactivity!!!', (10, frame_height - 20), self.font, 3, self.COLORS['blue'], 2, lineType=self.linetype)
                    play_sound = 'reset_counters'
                    self.state_tracker['start_inactive_time'] = time.perf_counter()
                    self.state_tracker['INACTIVE_TIME'] = 0.0

                
                cv2.putText(frame, str(int(left_hip_vertical_angle)), (left_hip_text_coord_x, hip_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(right_hip_vertical_angle)), (right_hip_text_coord_x, hip_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(left_knee_vertical_angle)), (left_knee_text_coord_x, left_knee_coord[1]+10), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(right_knee_vertical_angle)), (right_knee_text_coord_x, right_knee_coord[1]+10), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(left_ankle_vertical_angle)), (left_ankle_text_coord_x, ankle_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
                cv2.putText(frame, str(int(right_ankle_vertical_angle)), (right_ankle_text_coord_x, ankle_coord[1]), self.font, .5, self.COLORS['light_green'], 2, lineType=self.linetype)
            ...
            if self.state_tracker['INACTIVE_TIME'] >= self.thresholds['INACTIVE_THRESH']:
                self.state_tracker['KNEERAISE_COUNT'] = 0
                self.state_tracker['IMPROPER_KNEERAISE'] = 0
                cv2.putText(frame, 'Resetting KNEERAISE_COUNT due to inactivity!!!', (10, frame_height - 25), self.font, 0.7, self.COLORS['blue'], 2)
                display_inactivity = True
